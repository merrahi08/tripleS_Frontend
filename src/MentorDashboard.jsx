...
export default function MentorDashboard({ user, onLogout }) {
  ...
  const fetchLastConversationsSummary = async () => {
    ...
    try {
      ...
      console.error("Erreur lors du chargement du résumé des discussions:", error);
    }
  };
...
const processedClients = assignedClients.map((client) => {
    ...
    return {
      ...client,
      lastMessageTimestamp: lastMsg ? new Date(lastMsg.timestamp) : new Date(0),
      lastMessageText: lastMsg ? lastMsg.content : null,
      isLastMessageUnread: lastMsg ? (!lastMsg.isRead && msg.senderId === client.id) : false
    };
              ...
              <div className="bg-zinc-900/40 border border-purple-500/30 rounded-2xl p-5 space-y-4">
                <div className="flex justify-between items-center border-b border-white/5 pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-xs font-bold text-purple-400 uppercase tracking-wider">
                      Discussion en direct avec : <span className="text-white normal-case font-sans font-bold text-sm ml-1">{activeChatClient.name}</span>
                    </h3>
                  </div>
                  <button 
                ...
                <div className="h-48 bg-black/50 border border-white/5 rounded-xl p-3 overflow-y-auto space-y-2 text-xs">
                  {chatMessages.length === 0 ? (
                    <p className="text-zinc-600 text-center italic mt-12">Aucun historique disponible. Envoyez-lui un message d'accueil !</p>
                  ) : (
                    chatMessages.map((msg) => {
                      const isMe = msg.senderId === user.id;
                      return (
                        <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                          <div className={`max-w-[80%] rounded-lg px-3 py-1.5 line-clamp-none ${
                ...
                <form onSubmit={handleSendLiveMessage} className="flex gap-2">
                  ...
                  />
                  <button type="submit" className="px-4 py-1.5 bg-purple-600 hover:bg-purple-500 rounded-lg text-xs font-bold transition-all">
                    Envoyer
                  </button>
                </form>
              </div>
              ...
              {loading ? (
                ...
                <div className="space-y-4">
                  ...
                  {processedClients.map((client) => {
                    const isChattingWithThisClient = activeChatClient?.id === client.id;
                    const hasBadgeNotification = client.isLastMessageUnread && !isChattingWithThisClient;
                      ...
                      >
                        <div className="space-y-1 flex-1 min-w-0">
                          ...
                          {client.lastMessageText && (
                            <p className={`text-[11px] pt-1 mt-1 border-t border-white/5 truncate max-w-xl ${hasBadgeNotification ? 'text-purple-400 font-semibold' : 'text-zinc-500'}`}>
                              <span className="text-[10px] uppercase font-bold tracking-tight mr-1 opacity-80">Dernier échange :</span> 
                              {client.lastMessageText}
                            </p>
                          )}
                        </div>
                        ...
                        <div className="flex sm:flex-col gap-2 shrink-0 justify-end">
                          ...
                          >
                            💬 {isChattingWithThisClient ? 'Discussion Active' : hasBadgeNotification ? 'Nouveau message' : 'Envoyer un message'}
                          </button>
                        </div>
                      </div>
                    );
          ...
          <div className="space-y-6">
            <div className="bg-zinc-900/30 border border-white/5 rounded-2xl p-6">
              ...
              {loadingRequests ? (
                ...
                <p className="text-xs text-gray-500 italic py-2">Aucun message d'accompagnement en attente.</p>
              ) : (
                <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1">
                  {pendingRequests.map((req) => (
                    <div key={req.id} className="p-3 bg-zinc-900/80 border border-white/5 rounded-xl space-y-2 hover:border-purple-500/20 transition-all">
                      <div className="flex justify-between items-start gap-2">
                        <span className="text-xs font-bold text-gray-200 line-clamp-1">{req.subject || "Demande d'accompagnement"}</span>
                        <span className="text-[9px] font-bold bg-amber-500/10 border border-amber-500/20 text-amber-400 px-1.5 py-0.5 rounded uppercase tracking-wide shrink-0">{req.status}</span>
                      </div>
                      <p className="text-[11px] text-gray-400 leading-relaxed break-words">{req.description}</p>
