...
export default function App() {
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    // Create the precise payload structure expected by your Spring Boot backend entity mapping
    const payload = {
      name: formData.name,
      email: formData.email,
      password: formData.password || "DefaultInsecurePass123!", // Fallback password if not input
      projectIdea: formData.idea,
      selectedTier: selectedTier ? selectedTier.toUpperCase() : 'GRATUIT' // Formats tier to match Backend mapping
    };
    ...
    try {
      ...
      if (response.ok || response.status === 201) {
        ...
        console.log("Successfully registered user in PostgreSQL:", data);
        
        setRegisterSuccess(true);
        
        // Wait 1.5s for the user to see the success state, then close modal and login
        setTimeout(() => {
          setIsModalOpen(false);
          setRegisterSuccess(false);
          setFormData({ name: '', email: '', password: '', idea: '' });
          
          // CRUCIAL: Save the user record into React state to automatically show the dashboard view!
          setCurrentUser(data);
        }, 1500);
      } else {
        const errorText = await response.text();
        alert(`Erreur d'inscription: ${errorText || response.statusText}`);
      }
    } catch (error) {
      console.error("Connection failed:", error);
      alert("Impossible de contacter le serveur Spring Boot. Vérifiez qu'il tourne sur http://localhost:8080");
    }
  };
  ...
  // FIXED: The return keyword MUST have the opening parenthesis directly on its line or wrapping the block safely!
  return (
    <div className="min-h-screen bg-brand-black text-white selection:bg-brand-gold selection:text-brand-black">
      {currentUser ? (
        /* ─── 📊 VUE TABLEAU DE BORD (S'affiche si l'utilisateur est connecté) ─── */
        <div className="min-h-screen bg-brand-black text-white p-8 w-full">
          <header className="flex justify-between items-center border-b border-white/5 pb-4 mb-8">
            <div>
              ...
              <h1 className="text-3xl font-bold font-serif">Bienvenue, {currentUser.name} !</h1>
            </div>
          ...
          <main className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-brand-darkGray/40 border border-white/5 rounded-2xl p-6">
              ...
              <p className="text-gray-200 text-sm italic leading-relaxed">
                "{currentUser.projectIdea || "Aucune description fournie."}"
              </p>
            </div>
            ...
            <div className="bg-brand-darkGray/40 border border-white/5 rounded-2xl p-6">
              ...
              <div className="space-y-2 text-sm text-gray-300">
                <p>🟢 Compte activé</p>
                <p>{currentUser.selectedTier === 'PREMIUM' ? '✨ Accès aux Mentors Débloqué' : '🔒 Passez à Premium pour contacter les mentors'}</p>
              </div>
            </div>
          </main>
        </div>
      ) : (
        /* ─── 🌐 VUE VITRINE / LANDING PAGE (S'affiche par défaut) ─── */
              ...
              <div className="max-w-7xl mx-auto">
                <div className="text-center mb-16">
                  ...
                  <div className="flex items-center justify-center gap-4 mt-8">
                    ...
                    <button onClick={() => setIsAnnual(!isAnnual)} className="w-12 h-6 rounded-full bg-brand-darkGray border border-brand-gold/30 p-1 flex items-center transition-all">
            ...
            {isChatOpen ? (
              <div className="w-96 max-w-[calc(100vw-32px)] h-[500px] bg-brand-darkGray border border-brand-gold/30 rounded-2xl shadow-2xl flex flex-col justify-between overflow-hidden">
                <div className="bg-brand-black border-b border-white/5 p-4 flex items-center justify-between">
                  ...
                  <button onClick={() => setIsChatOpen(false)} className="text-gray-400 hover:text-white"><i className="fa-solid fa-xmark"></i></button>
                </div>
              ...
              <button onClick={() => setIsChatOpen(true)} className="w-14 h-14 rounded-full bg-brand-lightGold text-brand-black shadow-xl flex items-center justify-center hover:scale-105 transition-all text-xl">
              ...
              <div className="bg-brand-darkGray border border-brand-gold/30 rounded-3xl w-full max-w-md p-8 relative z-10">
                ...
                {registerSuccess ? (
                  <div className="text-center py-8">
                    ...
                    <h3 className="text-2xl font-bold text-white">Demande Enregistrée !</h3>
                    <p className="text-gray-400 text-sm mt-2">Votre profil a été synchronisé en base de données.</p>
                  </div>
                ) : (
                  <div>
                    <h3 className="text-2xl font-bold text-white mb-6">Rejoindre Triple S ({selectedTier})</h3>
